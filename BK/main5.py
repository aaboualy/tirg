...
def build_and_train_netCOS(hidden1,hidden2,max_iterations, min_error, all_queries,all_imgs,batch_size):
 ...
 #loss.backward()
  ...
  optimizer=torch.optim.SGD(model.parameters(), lr=0.015)
  epoch=max_iterations
  model.load_state_dict(torch.load(Path1+r"/"+r'\NLP4COS172K3000.pth'))  #NLP4COS172K3000

  losses=np.zeros((15000))
  for j in range(epoch):
    total_loss=0
    for l in range(int(all_queries.shape[0]/batch_size)):
    ...
    losses[j]=total_loss
    if(j%1000==0):
      torch.save(model.state_dict(), Path1+r'\NLP41COS172K'+str(j)+'.pth') 

  print ('mean square loss',loss_fn(model.myforward(all_queries),all_imgs))  
  print('Finished Training')
  with open(Path1+r"/"+'loosses4.pkl', 'wb') as fp:
          pickle.dump( losses, fp)

  torch.save(model.state_dict(), Path1+r'\NLP41COSfinal172k.pth') 
...
def neural_model(all_queries,all_imgs,model_option,test_queries):
  if model_option==0:
    ...
    if model_option==1:
      ...
      model.load_state_dict(torch.load(Path1+r"/"+r'\NLP4COSfinal172k.pth'))  #NLP4COS172K3000
